...
{
        ui.pageSpinBox->setPrefix(QString::number(pages)+" de ");
...
void TDPreviewDialog::exportPdf()
{
        // file save dialog
        QString dialogcaption = "Export PDF";
        QString exportname = QFileDialog::getSaveFileName(this, dialogcaption, titulopdf, "*.pdf");
...
void TDPreviewDialog::paintPage(int pagenum)
{
        ...
        if (footerSize > 0) {
                ...
                QGraphicsTextItem *item=new QGraphicsTextItem();
                item->setFont(font);
                item->setPlainText(footerText+QString::number(pagenum)+" de "+QString::number(pages)+"                                                  Pandasot 2010");
